...
#ifdef EUROPE_AMERICA_STANDARD_COMPATIBLE
#define SWITCH_EUROPE_AMERICA_TIMER (HZ/2)

struct timer_list switch_europe_america_timer;
static int switch_europe_america_timerout = 0;
static int switch_europe_america_flag = 0;
static int switch_europe_america_stable = 0;

static void switch_to_america(void)
{
...
static void switch_to_europe(void)
{
	mt_set_gpio_mode(GPIO_FSA8049_PIN, GPIO_FSA8049_PIN_M_GPIO);
	mt_set_gpio_dir(GPIO_FSA8049_PIN, GPIO_DIR_OUT);
...
static void switch_europe_america(unsigned long a)
{
    ...
    ACCDET_DEBUG("[Accdet]switch_europe_america:[%s]current AB = %d\n", 
		accdet_status_string[accdet_status], current_status);
    
    if(current_status == 0)
    {
        if(switch_europe_america_flag == 0)
        {
            switch_to_america();
            switch_europe_america_flag = 1;
        }
        else
        {
            switch_to_europe();
            switch_europe_america_flag = 0;
        }
    ...
    if(stable_count>=4)
    {
        switch_europe_america_stable = 1;
        stable_count = 0;
    }
    else
    {
        switch_europe_america_stable = 0;

        if(!switch_europe_america_timerout)
           mod_timer(&switch_europe_america_timer, jiffies + SWITCH_EUROPE_AMERICA_TIMER); 
    }
...
static void disable_micbias_callback(struct work_struct *work)
{
        ...
        if(cable_type == HEADSET_NO_MIC) {
			...
			#endif
                disable_accdet();
                ACCDET_DEBUG("[Accdet] more than 6s MICBIAS : Disabled\n");
        }
	#ifdef ACCDET_PIN_RECOGNIZATION
		else if(cable_type == HEADSET_MIC) {       
			pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width);
    		pmic_pwrap_write(ACCDET_PWM_THRESH, cust_headset_settings->pwm_thresh);
			ACCDET_DEBUG("[Accdet]pin recog after 6s recover micbias polling!\n");
		}
	#endif
    
#ifdef EUROPE_AMERICA_STANDARD_COMPATIBLE
    switch_europe_america_timerout = 1; 
    ACCDET_DEBUG("[Accdet]switch_europe_america_timerout!!\n");
#endif
}
...
//Disable ACCDET
		...
		del_timer_sync(&micbias_timer);
#ifdef EUROPE_AMERICA_STANDARD_COMPATIBLE
        del_timer_sync(&switch_europe_america_timer);
        switch_to_america();
        switch_europe_america_flag = 1;
        send_key_event(cur_key, 0);
        
        switch_europe_america_timerout = 0;
        switch_europe_america_stable = 0;
#endif        
		#ifdef ACCDET_PIN_RECOGNIZATION
		  show_icon_delay = 0;
...
{
#ifdef EUROPE_AMERICA_STANDARD_COMPATIBLE
    if((switch_europe_america_stable == 0) && (switch_europe_america_timerout == 0))
    {
        ACCDET_DEBUG("[accdet]accdet not stable, so not report key!!\n");
        return;
    }
#endif
...
#ifdef EUROPE_AMERICA_STANDARD_COMPATIBLE
    init_timer(&switch_europe_america_timer);
	switch_europe_america_timer.expires = jiffies + SWITCH_EUROPE_AMERICA_TIMER;
	switch_europe_america_timer.function = &switch_europe_america;
	switch_europe_america_timer.data = ((unsigned long) 0 );
