...
#define PLATFORM_DRIVER_NAME "lens_actuator_wv511aaf"
#define AF_DRIVER_CLASS_NAME "actuatordrv_wv511aaf"
...
inline static int getAFInfo(__user stWV511AAF_MotorInfo * pstMotorInfo)
{
    stWV511AAF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stWV511AAF_MotorInfo)))
    {
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stWV511AAF_MotorMETAInfo * pstMotorMETAInfo)
{
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case WV511AAFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stWV511AAF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case WV511AAFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stWV511AAF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
        case WV511AAFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;

        case WV511AAFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case WV511AAFIOC_T_SETMACROPOS :
...
static int __init WV511AAF_i2C_init(void)
{
...
static void __exit WV511AAF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(WV511AAF_i2C_init);
module_exit(WV511AAF_i2C_exit);

MODULE_DESCRIPTION("WV511AAF lens module driver");
