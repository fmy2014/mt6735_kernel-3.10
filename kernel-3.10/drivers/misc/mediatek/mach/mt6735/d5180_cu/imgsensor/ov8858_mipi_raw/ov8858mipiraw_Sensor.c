...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0xca71b5d,		//checksum value for Camera Auto Test
...
{{ 3296, 2480,	  12,	12, 3272, 2456, 3264, 2452,   4,	2, 1632, 1224,	 0, 0, 1632, 1224}, // Preview 
 ...
 { 3296, 2480,	  12,	12, 3272, 2456, 3264, 2452,   4,	2,  640,  480,	 0, 0,  640,  480}, //hight speed video 
...
//  ;;
	...
	write_cmos_sensor(0x302 , 0x23); //2lane 460M MIPICLK  //deto 0317 0x27  //0x20  //0x23 is OK! //
	write_cmos_sensor(0x303 , 0x00);
	write_cmos_sensor(0x304 , 0x03);
	write_cmos_sensor(0x30d , 0x14); //deto 0317  0x1f
	...
	write_cmos_sensor(0x3018, 0x32); //change to 2lane
	...
	//test start
	/*
	u32 fmyreg = read_cmos_sensor(0x302);
	u32 fmyreg2 = read_cmos_sensor(0x304);
	u32 fmyreg3 = read_cmos_sensor(0x5e01);
	
	u32 fmysensorid = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));

	printk("==========0317fmy reg = %x ==========\n",fmyreg);
	printk("==========0317fmy reg = %x ==========\n",fmyreg);
	printk("==========0317fmy reg = %x ==========\n",fmyreg);
	printk("==========0317fmy reg2 = %x ==========\n",fmyreg2);
	printk("==========0317fmy reg2 = %x ==========\n",fmyreg2);
	printk("==========0317fmy reg3 = %x ==========\n",fmyreg3);
	printk("==========0317fmy reg3 = %x ==========\n",fmyreg3);
	printk("==========0317fmysensorID = %x ==========\n",fmysensorid);
	printk("==========0317fmysensorID = %x ==========\n",fmysensorid);
	printk("==========0317fmysensorID = %x ==========\n",fmysensorid);
	printk("==========0317fmysensorID = %x ==========\n",fmysensorid);
	*/
...
//   100 99 1632 1224
	write_cmos_sensor(0x100 , 0x00);     //	
	write_cmos_sensor(0x30e , 0x02);     //
	write_cmos_sensor(0x30d , 0x14);     // 30fps   //deto 0317  0x1f
	write_cmos_sensor(0x312 , 0x03);     //
	write_cmos_sensor(0x3015, 0x00);     //
	write_cmos_sensor(0x3700, 0x18);     //
	write_cmos_sensor(0x3701, 0x0c);     //
...
//	write_cmos_sensor(0x3821, 0x67);     //
	...
	mdelay(10);
//test start
	printk("after-preview-fmy-start\n");
	u32 fmyreg = read_cmos_sensor(0x302);
	u32 fmyreg2 = read_cmos_sensor(0x304);
	u32 fmyreg3 = read_cmos_sensor(0x5e01);
...
//  100 99 3264 2448);    //
	...
	write_cmos_sensor(0x30d , 0x14);  // 15fps
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = ov8858R2A\n", imgsensor.i2c_write_id,*sensor_id);	
				return ERROR_NONE;
				}
				else if((read_cmos_sensor(0x302A)) == 0XB1){
 				ov8858version = OV8858R1A;
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8858version = ov8858R1A\n", imgsensor.i2c_write_id,*sensor_id); 
				return ERROR_NONE;
				}
				else{
