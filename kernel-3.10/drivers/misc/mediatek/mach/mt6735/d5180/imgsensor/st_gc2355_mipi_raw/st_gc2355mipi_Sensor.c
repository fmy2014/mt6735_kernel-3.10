...
#include "st_gc2355mipi_Sensor.h"

#define ST_GC2355MIPI_2Lane   //   for  mipi lane num
/****************************Modify Following Strings for Debug****************************/
#define PFX "ST_GC2355_camera_sensor"
#define LOG_1 LOG_INF("ST_GC2355,MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = ST_GC2355_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    //.checksum_value = 0x9d1c9dad,        //checksum value for Camera Auto Test
    .checksum_value = 0xfa9ae6af,        //checksum value for Camera Auto Test
  ...
  #if defined (ST_GC2355MIPI_2Lane)
...
static void set_dummy(void)
{
	...
	hb = imgsensor.dummy_pixel + ST_GC2355_DEFAULT_DUMMY_PIXEL_NUMS;
	vb = imgsensor.dummy_line + ST_GC2355_DEFAULT_DUMMY_LINE_NUMS;
...
//			LOG_INF("ST_GC2355 analogic gain 1x , ST_GC2355 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_2<= iReg)&&(iReg < ANALOG_GAIN_3))
	{
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
//			LOG_INF("ST_GC2355 analogic gain 1.45x , ST_GC2355 add pregain = %d\n",temp);
	...
	else if((ANALOG_GAIN_3<= iReg)&&(iReg < ANALOG_GAIN_4))
	{
			...
			write_cmos_sensor(0xb1, temp>>6);
			write_cmos_sensor(0xb2, (temp<<2)&0xfc);
//			LOG_INF("ST_GC2355 analogic gain 2.02x , ST_GC2355 add pregain = %d\n",temp);
	}
...
//			LOG_INF("ST_GC2355 analogic gain 2.8x, temp = %d\n", temp);
	}
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	//		ST_GC2355_write_cmos_sensor(0x92,0x03);
	//		ST_GC2355_write_cmos_sensor(0x94,0x0b);
			break;
		case IMAGE_V_MIRROR://IMAGE_V_MIRROR:
			write_cmos_sensor(0x17,0x16);
	//		ST_GC2355_write_cmos_sensor(0x92,0x02);
	//		ST_GC2355_write_cmos_sensor(0x94,0x0b);
			break;
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
    ...
    if(enable)
    {
        LOG_INF("[GC2355SetTestPatternMode]test pattern bEnable:=%d\n",enable);
         write_cmos_sensor(0xfe,0x00);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 retry = 2;
    int comp = -1;
	               mt_set_gpio_mode(GPIO_CAMERA_SUB_ID_PIN,GPIO_CAMERA_SUB_ID_PIN_M_GPIO);
		  mt_set_gpio_dir(GPIO_CAMERA_SUB_ID_PIN,GPIO_DIR_IN);
		     	
		  mt_set_gpio_pull_enable(GPIO_CAMERA_SUB_ID_PIN, GPIO_PULL_ENABLE);
	      mt_set_gpio_pull_select(GPIO_CAMERA_SUB_ID_PIN, GPIO_PULL_DOWN);
					
		 comp =  mt_get_gpio_in(GPIO_CAMERA_SUB_ID_PIN);
		     	printk("liupan comp = %d\n",comp);
	 if (comp != 1 ) {
...
UINT32 ST_GC2355_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    ST_GC2355_MIPI_RAW_SensorInit    */
