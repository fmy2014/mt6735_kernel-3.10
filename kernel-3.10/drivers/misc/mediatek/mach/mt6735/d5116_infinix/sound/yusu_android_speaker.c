...
bool Speaker_Init(void)
{
   ...
   mt_set_gpio_mode(GPIO_SPK_AMP_EN,GPIO_MODE_00);  // gpio mode
   mt_set_gpio_pull_enable(GPIO_SPK_AMP_EN,GPIO_PULL_ENABLE);
   mt_set_gpio_dir(GPIO_SPK_AMP_EN,GPIO_DIR_OUT); // output
   mt_set_gpio_out(GPIO_SPK_AMP_EN,GPIO_OUT_ZERO); // low
   PRINTK("-Speaker_Init Success");
   return true;
}
...
void Sound_Speaker_Turnon(int channel)
{
    ...
    mt_set_gpio_dir(GPIO_SPK_AMP_EN,GPIO_DIR_OUT); // output
/*******************set mode************************************/
    mt_set_gpio_out(GPIO_SPK_AMP_EN,GPIO_OUT_ONE); // high
    udelay(2);
    mt_set_gpio_out(GPIO_SPK_AMP_EN,GPIO_OUT_ZERO); // low
    udelay(2);
    mt_set_gpio_out(GPIO_SPK_AMP_EN,GPIO_OUT_ONE); // high
