...
#define PFX "DJ_OV8865_2lane_camera_sensor"
...
extern bool dj_otp_update_lenc();
extern void dj_otp_apply_lenc();
extern bool dj_otp_update_wb();
extern bool dj_otp_apply_wb();
bool dj_otp_flag=0;

  kal_uint16 DJ_OV8865_read_cmos_sensor(kal_uint32 addr)
{
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	mirror= DJ_OV8865_read_cmos_sensor(0x3820);
	flip  = DJ_OV8865_read_cmos_sensor(0x3821);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	//mt_set_gpio_in(GPIO_MAIN_CAM_ID_PIN,GPIO_DIR_OUT);
	mt_set_gpio_pull_enable(GPIO_MAIN_CAM_ID_PIN, GPIO_PULL_ENABLE);
	mt_set_gpio_pull_select(GPIO_MAIN_CAM_ID_PIN, GPIO_PULL_DOWN);
	//mt_set_gpio_pull_select(GPIO_MAIN_CAM_ID_PIN, GPIO_PULL_UP);
	comp =  mt_get_gpio_in(GPIO_MAIN_CAM_ID_PIN);
	printk("comp = %d\n",comp);
	if(comp != 1 )
	{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((DJ_OV8865_read_cmos_sensor(0x300B) << 8) | DJ_OV8865_read_cmos_sensor(0x300C));
...
static kal_uint32 open(void)
{
	...
	sensor_init();
	if(dj_otp_update_wb()&&dj_otp_update_lenc())
	{
		dj_otp_flag = 1;
		LOG_INF("OTP?¨®??3¨¦1|");
		dj_otp_update_wb();
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_GET_REGISTER:
            sensor_reg_data->RegData = DJ_OV8865_read_cmos_sensor(sensor_reg_data->RegAddr);
            break;
