...
static void torch_enable()
{
    mt_set_gpio_out(GPIO_ENT,GPIO_OUT_ONE);
    mt_set_gpio_out(GPIO_ENF,GPIO_OUT_ONE);
}

static void torch_disable()
{
    mt_set_gpio_out(GPIO_ENT,GPIO_OUT_ZERO);
	mt_set_gpio_out(GPIO_ENF,GPIO_OUT_ZERO);
}
...
static ssize_t flashlight_torch_enable_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    int enable = 0;
    if(buf != NULL && size != 0)
    {
...
static DEVICE_ATTR(flashlight_torch_enable, 0644, NULL, flashlight_torch_enable_store);
static int __init flashlight_torch_init(void)  
{		
	flashlight_torch_class = class_create(THIS_MODULE, "flashlight_torch");
	flashlight_torch_dev = device_create(flashlight_torch_class,NULL, 0, NULL,  "flashlight_torch");
    device_create_file(flashlight_torch_dev, &dev_attr_flashlight_torch_enable);
	return 0;
}
static void __exit flashlight_torch_exit(void)
{
	return;
}
module_init(flashlight_torch_init);
module_exit(flashlight_torch_exit);
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("flashlight_torch");
