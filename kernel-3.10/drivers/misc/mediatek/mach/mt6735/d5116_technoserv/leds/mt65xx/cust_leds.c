...
//rgb_r
static void cust_rgb_led_r_set(unsigned int level)
{
    if(level)
    {
        mt_set_gpio_mode(GPIO_RGB_LED_R_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_RGB_LED_R_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_RGB_LED_R_PIN, GPIO_OUT_ONE);
    }
    else
    {
        mt_set_gpio_mode(GPIO_RGB_LED_R_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_RGB_LED_R_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_RGB_LED_R_PIN, GPIO_OUT_ZERO);   
    }
}

//rgb_g
static void cust_rgb_led_g_set(unsigned int level)
{
    if(level)
    {
        mt_set_gpio_mode(GPIO_RGB_LED_G_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_RGB_LED_G_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_RGB_LED_G_PIN, GPIO_OUT_ONE);
    }
    else
    {
        mt_set_gpio_mode(GPIO_RGB_LED_G_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_RGB_LED_G_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_RGB_LED_G_PIN, GPIO_OUT_ZERO);   
    }
}

//rgb_b
static void cust_rgb_led_b_set(unsigned int level)
{
    if(level)
    {
        mt_set_gpio_mode(GPIO_RGB_LED_B_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_RGB_LED_B_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_RGB_LED_B_PIN, GPIO_OUT_ONE);
    }
    else
    {
        mt_set_gpio_mode(GPIO_RGB_LED_B_PIN, GPIO_MODE_00);
        mt_set_gpio_dir(GPIO_RGB_LED_B_PIN, GPIO_DIR_OUT);
        mt_set_gpio_out(GPIO_RGB_LED_B_PIN, GPIO_OUT_ZERO);   
    }
}
...
static struct cust_mt65xx_led cust_led_list[MT65XX_LED_TYPE_TOTAL] = {
	...
	{"blue",              MT65XX_LED_MODE_GPIO, cust_rgb_led_b_set,{0}},
