...
u32 pinSet[3][8] = {
    ...
    if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2755_MIPI_RAW,currSensorName)))
        ...
        {
                PK_DBG("XXXXXXXXXX SP2508 sensor PDN off is one\n");
                pinSet[1][IDX_PS_CMPDN + IDX_PS_ON] = GPIO_OUT_ZERO;
                pinSet[1][IDX_PS_CMPDN + IDX_PS_OFF] = GPIO_OUT_ONE;
        }
        ...
        if(currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2755_MIPI_RAW,currSensorName)))
        {
            if(TRUE != _hwPowerOn(CAMERA_POWER_VCAM_IO, VOL_1800,mode_name))
                ...
                if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
                {
                        ...
                        mdelay(5);
                        if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set IDX_PS_CMPDN failed!! \n");}
                        mdelay(5);
                        if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){PK_DBG("[CAMERA LENS] set IDX_PS_CMPDN failed!! \n");}
                        mdelay(5);
                        ...
                        mdelay(5);
                        if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set IDX_PS_CMRST failed!! \n");}
                        mdelay(5);
                        if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_ON])){PK_DBG("[CAMERA SENSOR] set IDX_PS_CMRST failed!! \n");}
                        mdelay(5);
                }
...
/* HI545 START */
        ...
        {
////////////////////////// FIXME:the same as hi545????? ///////////////////////////
           PK_ERR( "TURN ON hi545!@%d start!", __LINE__ );
            ...
            if  ( GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST] ) {
                if (mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE]))
                    PK_DBG("RESET_PIN mode failed!! \n");

                if (mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_DIR_OUT))
                    PK_DBG("set RESET_PIN out direction failed!! \n");

                if (mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_OUT_ZERO))
                    PK_DBG("set XSHUNTDOWN_PIN = GPIO_OUT_ZERO failed!! \n");

                if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],GPIO_OUT_ONE))
                    PK_DBG("set XSHUNTDOWN_PIN = GPIO_OUT_ONE failed!! \n");
            }
          ...
          if(pinSetIdx == 0 || pinSetIdx == 2) {//disable sub
              ...
              if (GPIO_CAMERA_INVALID != pinSet[0][IDX_PS_CMRST]) {
                ...
                PK_ERR( "TURN ON hi545!@%d end", __LINE__ );
        }
/* HI545 END */
        ...
        else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP2508_MIPI_RAW,currSensorName)))
        {
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                ...
                if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");} //high == pwnd sensor
            }
...
/* HI545 START */
        ...
        {
            PK_ERR("POWER DOWN hi545!@%d", __LINE__ );

            //reset pull down: RST = 0 to power off
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_MODE])) {
            ...
            //PDN pull down: PDN = 0 to power off
            if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
                if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_MODE])) {
                        PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");
                }
