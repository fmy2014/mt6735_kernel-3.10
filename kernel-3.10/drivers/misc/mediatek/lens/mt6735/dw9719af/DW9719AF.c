...
#define AF_DRVNAME "DW9719AF"

#define I2C_SLAVE_ADDRESS        	0x18
#define I2C_REGISTER_ID            	0x23
#define PLATFORM_DRIVER_NAME "lens_actuator_dw9719af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_dw9719af"
...
u8 read_data(u8 addr)
{
    ...
    LOG_INF("[DW9719AF]  get_byte %d \n",  get_byte);
    return get_byte;
}


static void s4DW9719AF_get_ic_info(void)
{
    LOG_INF("[DW9719AF]  ic_info = %d \n",  read_data(0x00));
}

static void s4DW9719AF_get_ic_version(void)
{
    LOG_INF("[DW9719AF]  ic_version = %d \n",  read_data(0x01));
}

static void s4DW9719AF_set_ring_mode(void)
{
    int  i4RetValue = 0;

/* mode 1 is recommended */
    // setp 1: [0x02] = 0x02
    char puSendCmd[2];
    ...
    if (i4RetValue < 0)
    {
        LOG_INF("[DW9719AF] I2C send failed!! \n");
        return -1;
    }

    // setp 2: [0x06] = 0x40 (SAC mode 3 setting)
    puSendCmd[0] = 0x06;
    puSendCmd[1] = 0x40;
    ...
    // setp 3: [0x07] = 0x65 (PS[1:0] = 01, RC[5:0] = 100101, Res. freq = 100Hz, Tvib = 10.0ms )
    puSendCmd[0] = 0x07;
    puSendCmd[1] = 0x65;
    ...
    LOG_INF("[DW9719AF] set ring mode successfully  !! \n");
    return i4RetValue;
}
...
@ 0: not busy  -1: BUSY
*/
static int s4DW9719AF_is_busy()
{
    if (read_data(0x05) & 0x01) {
      LOG_INF("[DW9719AF]  busy! \n");
      return -1;
    } else {
      return 0;
    }
...
static int s4AF_ReadReg(unsigned short * a_pu2Result)
{
    ...
    *a_pu2Result = ((read_data(0x03) & 0X03) << 8) | (read_data(0x04));

    LOG_INF("[DW9719AF]  s4AF_ReadReg %d \n",  *a_pu2Result);
    return 0;
}
...
inline static int getAFInfo(__user stDW9719AF_MotorInfo * pstMotorInfo)
{
    ...
    {
        LOG_INF("[DW9719AF] copy to user failed when getting motor information \n");
    }
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stDW9719AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stDW9719AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stDW9719AF_MotorMETAInfo)))
    {
...
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    // if ring mode is on, must check busy bit
    if (g_ring_mode) {
      if (s4DW9719AF_is_busy()) {
        LOG_INF("[DW9719AF] can not move as SAC is busy! \n");
        return -EINVAL;
      }
    }
    ...
    LOG_INF("[DW9719AF] move [curr] %d [target] %d\n", (int)g_u4CurrPosition, (int)g_u4TargetPosition);
            ...
            {
                LOG_INF("[DW9719AF] set I2C failed when moving the motor \n");			
                spin_lock(&g_AF_SpinLock);
                g_i4MotorStatus = -1;
                spin_unlock(&g_AF_SpinLock);				
            }
...
printk("FMYFMYFMY-5-8---a_u4Command = %d\n",a_u4Command);

    switch(a_u4Command)
    {
        case DW9719AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stDW9719AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case DW9719AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stDW9719AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
...
{
    LOG_INF("[DW9719AF] AF_Open - Start\n");


    if(g_s4AF_Opened)
    {
        LOG_INF("[DW9719AF] the device is opened \n");
        return -EBUSY;
    }
    spin_lock(&g_AF_SpinLock);
    g_s4AF_Opened = 1;
    ...
    s4DW9719AF_get_ic_info();
    s4DW9719AF_get_ic_version();
    s4DW9719AF_set_ring_mode();
...
{
    LOG_INF("[DW9719AF] AF_Release - Start\n");

    if (g_s4AF_Opened)
    {
        LOG_INF("[DW9719AF] free \n");
        g_sr = 5;
	    s4AF_WriteReg(200);
        msleep(10);
	    s4AF_WriteReg(100);
    ...
    LOG_INF("[DW9719AF] AF_Release - End\n");
...
inline static int Register_AF_CharDrv(void)
{
    ...
    LOG_INF("[DW9719AF] Register_AF_CharDrv - Start\n");

    //Allocate char driver no.
    if( alloc_chrdev_region(&g_AF_devno, 0, 1,AF_DRVNAME) )
    {
        LOG_INF("[DW9719AF] Allocate device no failed\n");
    ...
    if(NULL == g_pAF_CharDrv)
    {
        ...
        LOG_INF("[DW9719AF] Allocate mem for kobject failed\n");
    ...
    {
        LOG_INF("[DW9719AF] Attatch file operation failed\n");
    ...
    LOG_INF("[DW9719AF] Register_AF_CharDrv - End\n");    
    return 0;
}

inline static void UnRegister_AF_CharDrv(void)
{
    LOG_INF("[DW9719AF] UnRegister_AF_CharDrv - Start\n");
    ...
    LOG_INF("[DW9719AF] UnRegister_AF_CharDrv - End\n");    
}
...
static const struct i2c_device_id i2c_device_id[] = {{AF_DRVNAME,0},{}};   
...
static int AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(i4RetValue){
        ...
        LOG_INF("[DW9719AF] register char device failed!\n");
