...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    dynk_thd_offset = 500; //If NEAR, change H/L offset
#if PS_DYN_K_STR
...
//	    APS_LOG("[%s]:restart dynk thre_l = %ld, thre_h = %ld\n", __func__, dynk_thd_low, dynk_thd_high);
#endif
        eint_flag = false;
...
void epl_sensor_enhance_enable(struct i2c_client *client, bool enable)
{
    ...
    if(enable == true){
        enh_time = als_sensing_time(dynk_enhance_integration_time, dynk_enhance_adc, epl_sensor.als.cycle);
...
void epl_sensor_dynk_thd_polling_work(struct work_struct *work)
{
    ...
    if(enable_ps == true)
    {
...
#endif
        {
            ...
            mutex_unlock(&sensor_mutex);
            eint_flag = true;
		    APS_LOG("[%s]:dyn k thre_l = %u, thre_h = %u\n", __func__, dynk_thd_low, dynk_thd_high);
        }
...
#endif
        len += snprintf(buf+len, PAGE_SIZE-len, "Dyn thr(L/H) = (%u/%u)\n", dynk_thd_low, dynk_thd_high);
#endif
