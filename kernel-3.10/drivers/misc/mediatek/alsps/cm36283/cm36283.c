...
//modify by yangjuwei 20140918 start 
#ifdef SW_CALI
	...
	*data = databuf[0];
#endif
//modify by yangjuwei 20140918 end
	return 0;
	READ_PS_EXIT_ERR:
	return res;
}
...
static int cm36283_get_als_value(struct cm36283_priv *obj, u16 als)
{
		...
		{
			unsigned long endt = atomic64_read(&obj->als_deb_end);
			if(time_after(jiffies, endt))
			{
				atomic_set(&obj->als_deb_on, 0);
			}
...
static ssize_t cm36283_ps_write_canc(int buf, size_t count)
{	
	u8 databuf[3];
	int res;
	...
	if(buf >= 50)          //add a MAX Calibration value for error writing
		databuf[1] = 0;
	else
		databuf[1] = buf;
...
static long cm36283_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
		...
		switch (cmd)
		{
			...
			case ALSPS_IOCTL_SET_CALI:
				...
				obj->ps_cali = ps_cali;
				#ifndef SW_CALI
				err = cm36283_ps_write_canc(ps_cali,1);//add by yangjuwei 20140918
				if(err <= 0)
				{
					err = -EFAULT;
					goto err_out;
				}
				APS_LOG("write ps canc : %d\n",ps_cali);

				if(obj->ps_cali < (int)(obj->hw->ps_threshold_low-5))
				{				
				...
				{
					atomic_set(&obj->ps_thd_val_high, (15));
					atomic_set(&obj->ps_thd_val_low,  (6));
				}
				set_psensor_threshold(obj->client);
				#endif
				break;
