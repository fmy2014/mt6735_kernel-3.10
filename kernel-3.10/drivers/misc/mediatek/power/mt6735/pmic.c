...
void exec_battery_percent_callback(BATTERY_PERCENT_LEVEL battery_percent_level) //0:no limit
{
    ...
    else
    {
      ...
      #else
//lisong 2015-6-8 [BUGID:NULL][fix bug:unregsiter flashlight low percent protect will lead to system crash]start      
        if(bpcb_tb[BATTERY_PERCENT_PRIO_FLASHLIGHT].bpcb != NULL){
//lisong 2015-6-8 [BUGID:NULL][fix bug:unregsiter flashlight low percent protect will lead to system crash]end            
...
ret = pmic_config_interface(0xF54,0x2,0x7,1); // [3:1]: RG_VBAT_OV_VTH; Tim:for 4.40/4.35 battery
#else
ret = pmic_config_interface(0xF54,0x0,0x7,1); // [3:1]: RG_VBAT_OV_VTH; Tim:for 4.20/4.15 battery
#endif
ret = pmic_config_interface(0xF62,0x3,0xF,0); // [3:0]: RG_CHRWDT_TD; Tim:WDT=32s
ret = pmic_config_interface(0xF6C,0x2,0x1F,0); // [4:0]: RG_LBAT_INT_VTH; Ricky: E1 only
ret = pmic_config_interface(0xF70,0x1,0x1,1); // [1:1]: RG_BC11_RST; Tim:Disable BC1.1 timer
