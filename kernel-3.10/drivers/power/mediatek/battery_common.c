...
static void mt_battery_Sync_UI_Percentage_to_Real(void)
{
	...
	if ((BMT_status.UI_SOC > BMT_status.SOC) && ((BMT_status.UI_SOC != 1))) {
		...
		#if !defined(CUST_CAPACITY_OCV2CV_TRANSFORM)
#if 1//lisong 2015-5-7 [BUGID:NULL][fix bug:the battery not full even capacity shows 100]start
        if(BMT_status.charger_exist == KAL_TRUE)
        {
            if(BMT_status.SOC == 100)
            {
                if(BMT_status.bat_full == KAL_TRUE){
                    BMT_status.UI_SOC = 100;
                    battery_log(BAT_LOG_CRTI, "[Sync_Real] is really full!!\n");
                }
                else
                {
                    BMT_status.UI_SOC = 99;
                    battery_log(BAT_LOG_CRTI, "[Sync_Real] is not really full!!\n");
                }
            }
            else
            {
        ...
        battery_log(BAT_LOG_CRTI, "[Sync_Real] UI_SOC=%d, SOC=%d\n",
				    BMT_status.UI_SOC, BMT_status.SOC);
#else
		BMT_status.UI_SOC = BMT_status.SOC;
#endif//lisong 2015-5-7 [BUGID:NULL][fix bug:the battery not full even capacity shows 100]end
		#else
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (resetBatteryMeter == KAL_TRUE) {
		...
		if (bat_is_recharging_phase() == KAL_TRUE) {
#if 1//lisong 2015-5-7 [BUGID:NULL][exit recharge mode if SOC less than 95]start 
            if(BMT_status.SOC < 95)
            {
                BMT_status.UI_SOC = BMT_status.SOC;
                BMT_status.bat_in_recharging_state = KAL_FALSE;
                BMT_status.bat_full = KAL_FALSE;
                
                battery_log(BAT_LOG_CRTI, "[recharging] UI_SOC=%d, SOC=%d, have to exit recharge mode!!\n",
					    BMT_status.UI_SOC, BMT_status.SOC);
            }
            else
            {
			...
			battery_log(BAT_LOG_CRTI, "[recharging] UI_SOC=%d, SOC=%d\n",
					    BMT_status.UI_SOC, BMT_status.SOC);
#endif//lisong 2015-5-7 [BUGID:NULL][exit recharge mode if SOC less than 95]end            
		} else {
...
//2015-4-3,add for charge disturb tp,start
#ifdef CONFIG_RGK_CHARGER_DISTURB_TOUCHPANEL
extern void tpd_usb_plugin(int plugin);
#endif
//2015-4-3,add for charge disturb tp,end
