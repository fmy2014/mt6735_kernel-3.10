...
static unsigned char CTPM_FW0[] = {
	#include "FT6336_ragenteck_3062_Ver0x13_ID67_20150210_app.i"
};
static unsigned char CTPM_FW1[] = {
	#include "FT6x36_coolpad5180_ID82_Ver0x22_20150507_app.i"
};
static unsigned char CTPM_FW2[] = {
	#include "FT6336_ragenteck_3062_Ver0x13_ID67_20150210_app.i"
};
static unsigned char CTPM_FW3[] = {
	#include "FT6336_ID82_ragenteckL_3062_V11_20150212_app.i"
};
...
//	off_t off, int count, int *eof, void *data )
{
	struct i2c_client *client = fts_proc_entry_i2c_client;	//(struct i2c_client *)fts_proc_entry->data;
	int ret = 0;
	unsigned char buf[128];//PAGE_SIZE
...
#define FTS_SCAP_INI_FILEPATH	"/system/etc/ft_rawdata.ini"
#define FT6X06_RAWDATA_SIZE		5*1024

static struct mutex g_rawdata_mutex;

static int ft6x06_GetInISize(void)
{
	struct file *pfile = NULL;
	struct inode *inode;
	unsigned long magic;
	...
	if (NULL == pfile)
		pfile = filp_open(FTS_SCAP_INI_FILEPATH, O_RDONLY, 0);

	if (IS_ERR(pfile)) {
		printk("ft6x06_GetInISize : error occured while opening file %s.\n", FTS_SCAP_INI_FILEPATH);
		return -EIO;
	}
...
static int ft6x06_ReadInIData(char *config_buf)
{
	struct file *pfile = NULL;
	struct inode *inode;
	unsigned long magic;
	...
	if (IS_ERR(pfile)) {
		...
		printk("xinan --FTS_SCAP_INI_FILEPATH  %s\n",FTS_SCAP_INI_FILEPATH);
	inode = pfile->f_dentry->d_inode;
	magic = inode->i_sb->s_magic;
	fsize = inode->i_size-1;
	printk("xinan -- fsize=%ld\n",fsize);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	//pos = 0;
	pos=pfile->f_pos;
...
static int ft6x06_get_testparam_from_ini(void)
{
	char *filedata = NULL;
	int inisize = ft6x06_GetInISize();

	printk("xinan --inisize = %d \n ", inisize);
	if (inisize <= 0) {
	...
	if (ft6x06_ReadInIData(filedata)) {
		printk("%s() - ERROR: request_firmware failed\n",
					__func__);
		kfree(filedata);
		return -EIO;
	} else {
		printk("xinan --ft6x06_ReadInIData successful\n");
	}
...
static ssize_t ft6x06_rawdata_result_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	char *s = buf;
printk("xinan --ft6x06_rawdata_result_show\n ");
	mutex_lock(&g_rawdata_mutex);

	Init_I2C_Write_Func(focal_i2c_Write);
	Init_I2C_Read_Func(focal_i2c_Read);

	if(ft6x06_get_testparam_from_ini() <0)
	{
...
static ssize_t ft6x06_rawdata_result_store(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	return -EPERM;
...
/*raw data show*/
static ssize_t ft6x06_rawdata_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	printk("xinan --ft6x06_rawdata_show begin\n");
	ssize_t ret_count = 0;
	char *databuf = NULL;

	databuf = kmalloc(FT6X06_RAWDATA_SIZE, GFP_ATOMIC);
	...
	memset(databuf, 0 , FT6X06_RAWDATA_SIZE);
	...
	{
			printk("xinan --ft6x06_rawdata_show start test\n");
		StartTestTP();
		FreeTestParamData();

		ret_count = focal_save_scap_sample(databuf, FT6X06_RAWDATA_SIZE);
...
static ssize_t ft6x06_rawdata_store(struct kobject *kobj,
...
static DEVICE_ATTR(rawdata, S_IRUGO | S_IWUSR, ft6x06_rawdata_show,
			ft6x06_rawdata_store);
static DEVICE_ATTR(rd_result, S_IRUGO | S_IWUSR, ft6x06_rawdata_result_show,
			ft6x06_rawdata_result_store);
...
static struct kobject *TP_info_kobj;

int ft6x06_rawdata_sysfs_init(void)
{ 
	TP_info_kobj = kobject_create_and_add("CTP", NULL);
	...
	return sysfs_create_group(TP_info_kobj, &ft6x06_attribute_group);
}

void ft6x06_rawdata_sysfs_exit(void)
{
	sysfs_remove_group(TP_info_kobj, &ft6x06_attribute_group);

	mutex_destroy(&g_rawdata_mutex);

	kobject_put(TP_info_kobj);
}
/**********RawData Check End***********************/
#endif
