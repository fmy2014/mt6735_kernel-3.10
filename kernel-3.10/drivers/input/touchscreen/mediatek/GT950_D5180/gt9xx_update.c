...
s32 gup_load_calibration(char *firmware, s32 length, u8 need_check, u8 load_init);
#endif
...
static u32 current_calibration_length = 4096;
...
s32 gup_load_by_addr(u16 addr, u8 need_check, u8 *fw, u32 length)
{
    s32 ret = FAIL;
    s32 retry = 0;

    GTP_DEBUG("[load_by_addr]begin load [addr:0x%04x,length:%d].", addr, length);
    GTP_DEBUG_ARRAY(fw, 100);
    while(retry++ < 5)
    {
        //switch to bank1
        gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__MEM_CD_EN, 1);
        ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 1);    
        if (ret <= 0)
        {
            GTP_ERROR("[load_by_addr]select bank fail.");
            continue;
        }
        
        ret = i2c_write_bytes(i2c_client_point, addr, fw, length);
        if (ret == -1)
        {
            GTP_ERROR("[load_by_addr]load code fail.");
            continue;
        }    
        
		if(need_check)
		{
			ret = gup_check_and_repair(i2c_client_point, addr, fw, length);
			if(FAIL == ret)
			{
				GTP_ERROR("[load_by_addr]checked code fail.");
				continue;
			}
		}
		
		//switch back to bank3
		ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 3);    
        if (ret <= 0)
        {
            GTP_ERROR("[load_by_addr]select bank fail.");
            continue;
        }
        gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__MEM_CD_EN, 0);
        break;
    }
    if(retry<5)
    {
...
void gup_load_init_code(void)
{
    GTP_DEBUG("Begin load 4k overlay init code.");
    gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 1);
    GTP_DEBUG("Write 0x01 to register 0x4049.");
    gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__MEM_CD_EN, 1);
    i2c_write_bytes(i2c_client_point, 
                    0xF000, 
                    &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH+0x7000], 
                    0x1000);
    gup_set_ic_msg(i2c_client_point, 0x8040, 0x40);
}

s32 gup_load_calibration(char *data, s32 length, u8 need_check, u8 load_init)
{
    s32 ret = -1;
    u8 bank = 0;
    //u8 cmd = 0;
	...
	gup_set_ic_msg(i2c_client_point, 0x4010, 0x00);
	gup_set_ic_msg(i2c_client_point, 0x40B0, 0x00);
	gup_set_ic_msg(i2c_client_point, 0x404B, 0x00);
	gup_set_ic_msg(i2c_client_point, 0x4190, 0x02);
	gup_set_ic_msg(i2c_client_point, 0x4184, 0x01);
	gup_set_ic_msg(i2c_client_point, 0x5094, 0x00);
	gup_set_ic_msg(i2c_client_point, 0x4218, 0x00);
	gup_set_ic_msg(i2c_client_point, 0x4049, 0x01);
	
	//overlay need load init code when recovery
    if(load_init)
    {
        GTP_DEBUG("Recovery load 4k overlay init code.");
        gup_load_init_code();
    }
    ...
    if(load_init)
    {
        GTP_DEBUG("Begin FW check!");
        gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 0);
        ret = gup_check_and_repair(i2c_client_point, 
                                   0xC000, 
                                   &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH], 
                                   FW_DOWNLOAD_LENGTH);
        if(FAIL == ret)
        {
            GTP_ERROR("Checked FW bank0 fail.");
        }
        GTP_DEBUG("FW bank0 check pass!");
        gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 1);
        ret = gup_check_and_repair(i2c_client_point, 
                                   0xC000, 
                                   &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH + BANK_LENGTH], 
                                   FW_DOWNLOAD_LENGTH);
        if(FAIL == ret)
        {
            GTP_ERROR("Checked FW bank1 fail.");
        }
        GTP_DEBUG("FW bank1 check pass!");
        gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 2);
        ret = gup_check_and_repair(i2c_client_point, 
                                   0xC000, 
                                   &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH + 2*BANK_LENGTH], 
                                   4096);
        if(FAIL == ret)
        {
            GTP_ERROR("Checked FW bank1 fail.");
        }
        GTP_DEBUG("FW DSP check pass!");
    }
    */
...
s32 gup_load_calibration2(void)
{
    ...
    ret = gup_load_calibration(&section[CALIBRATION_HEADER_LEN], length, 1, 0);
...
s32 gup_load_calibration1(void)
{
    ...
    */
    ...
    ret = gup_load_calibration(&section[CALIBRATION_HEADER_LEN], length, 0, 0);
...
s32 gup_recovery_calibration0(void)
{
    ...
    ret = gup_load_calibration(&section[CALIBRATION_HEADER_LEN], current_calibration_length, 0, 1);
...
s32 gup_load_overlay_system(u16 addr, u32 offset, u32 length)
{
    s32 ret = -1;
    u8 cmd = 0;
    u8 *buf = NULL;
    ...
    if(length == 0)
    {
        GTP_ERROR("[load_overlay_system]Invalid load length.");
        return FAIL;
    }
    
    //test
    i2c_read_bytes(i2c_client_point, 0x8041, &cmd, 1);
    GTP_DEBUG("Check 0x8041 state:0x%02x", cmd);
    //test
    i2c_read_bytes(i2c_client_point, 0x8041, &cmd, 1);
    GTP_DEBUG("Check 0x8041 state:0x%02x", cmd);
    cmd = 0x00;
    //gup_set_ic_msg(i2c_client_point, GTP_REG_RQST, cmd);
    i2c_write_bytes(i2c_client_point, 0x8043, &cmd, 1);
    
    while(retry++ < 5)
    {
        msleep(10);
        ret = i2c_read_bytes(i2c_client_point, 0x8043, &cmd, 1);
        if(ret != 0)
        {
            continue;
        }
        if(0x0B == cmd)
        {
            GTP_DEBUG("[load_overlay_system]Chip ready to receive.");
            break;
        }
        GTP_DEBUG("Check 0x8043 state:0x%02x", cmd);
    }
    
    if(retry >= 5)
    {
        GTP_ERROR("[load_overlay_system]Wait for chip ready timeout failed.");
        return FAIL;
    }
    ...
    while(retry++ < 5)
    {
        buf = (u8*)kzalloc(length, GFP_KERNEL);
        if(NULL != buf)
        {
            GTP_DEBUG("[load_overlay_system]Kzalloc %d bytes memory success.", length);
            break;
        }
    }
    ...
    if(retry >= 5)
    {
        GTP_ERROR("[load_overlay_system]Kzalloc %d bytes memory failed.", length);
        return FAIL;
    }
    
    memcpy(buf, &gtp_hotknot_calibration_section0[FW_HEAD_LENGTH+offset], length);
#if GTP_ESD_PROTECT
    gtp_esd_switch(i2c_client_point, SWITCH_OFF);
#endif 
    ...
    //close WDT
    cmd = 0x00;
    i2c_write_bytes(i2c_client_point, 0x40B0, &cmd, 1);
    i2c_write_bytes(i2c_client_point, 0x404B, &cmd, 1);
    i2c_write_bytes(i2c_client_point, 0x40B4, &cmd, 1);
    i2c_write_bytes(i2c_client_point, 0x40B8, &cmd, 1);
    //cmd = 0x08;
    //i2c_write_bytes(i2c_client_point, 0x4180, &cmd, 1);
    //close CLK
    i2c_read_bytes(i2c_client_point, 0x4010, &cmd, 1);
    cmd |= 0x78;
    i2c_write_bytes(i2c_client_point, 0x4010, &cmd, 1);
    
    ret = gup_load_by_addr(addr, 0, buf, length);
    if(FAIL == ret)
    {
        GTP_ERROR("[load_overlay_system]Load overlay by addr failed.");
        goto exit_load_overlay;
    }
    ...
    //open CLK
    i2c_read_bytes(i2c_client_point, 0x4010, &cmd, 1);
    GTP_DEBUG("Read clk value(0x4010): 0x%02x", cmd);
    msleep(10);
    cmd &= (~0x78);
    i2c_write_bytes(i2c_client_point, 0x4010, &cmd, 1);
    cmd = 0;
    i2c_read_bytes(i2c_client_point, 0x4010, &cmd, 1);
    GTP_DEBUG("CLK REG(0x4010) value:0x%02x", cmd);
    if(0x78 == cmd)
    {
        GTP_DEBUG("Open clk again.");
        cmd &= (~0x78);
        i2c_write_bytes(i2c_client_point, 0x4010, &cmd, 1);
    }
    ...
    cmd = 0x00;
    i2c_write_bytes(i2c_client_point, GTP_REG_RQST, &cmd, 1);
    
    ret = SUCCESS;
    GTP_DEBUG("[load_overlay_system]Load success.");
...
s32 gup_recovery_from_gesture(u8 type)
{
    s32 ret = -1;
    char *firmware = NULL;
    u32 length = 0;

    GTP_INFO("[recovery_from_gesture] Recovery from gesture system."); 
    ...
    firmware = &gtp_hotknot_calibration_section0[0x4800 + FW_HEAD_LENGTH];
    
    ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 1);    
    if (ret <= 0)
    {
        GTP_ERROR("[recovery_from_gesture]select bank 1 fail.");
        goto recovery_gesture_exit;
    }
        
    ret = i2c_write_bytes(i2c_client_point, 0xC800, firmware, length);
    if (ret == -1)
    {
        GTP_ERROR("[recovery_from_gesture]download 4k code fail.");
        goto recovery_gesture_exit;
    }    
    ...
    {
        length = 4*1024;
        firmware = &gtp_hotknot_calibration_section0[0x8000 + FW_HEAD_LENGTH];
        
        ret = gup_set_ic_msg(i2c_client_point, _bRW_MISCTL__SRAM_BANK, 2);    
        if (ret <= 0)
        {
            GTP_ERROR("[recovery_from_gesture]select bank 2 fail.");
            goto recovery_gesture_exit;
        }
        ...
        if (ret == -1)
        {
            GTP_ERROR("[recovery_from_gesture]download 4k dsp code fail.");
            goto recovery_gesture_exit;
        }
        ...
        if(FAIL == ret)
        {
            GTP_ERROR("[recovery_from_gesture]checked FW bank fail.");
            goto recovery_gesture_exit;
        }
        */
